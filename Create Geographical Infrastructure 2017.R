...
#INPUTS  PATHS 
PA17path =      "/Users/henrygomory/Documents/Research/BARI/Git/New-BARI/Property Assessment 2017/PADCross.Record.2017.csv"
...
walkover_path = "Documents/Research/BARI/Git/New-BARI/Geographical Infrastructure 2017/IDConnector.2017.csv"

# READ IN INPUTS
roadsShp_b = getRoadsShp()
blocksShp_b = getBlocksShp()
PA17_b = read.csv(PA17path, stringsAsFactors=F)
P17shp_b = readOGR(P17shp_path,P17shp_name,stringsAsFactors=F)
mal_b = read.csv(malpath,stringsAsFactors=F)

# MAKE BACKUPS FOR RERUN
# this is not really necessary, I just did this while coding the syntax so that I could rerun the code from the beginning 
# without having to read the files back in, which was time consuming
PA17  = PA17_b
P17shp = P17shp_b
mal = mal_b
blocksShp = blocksShp_b
roadsShp = roadsShp_b 
...
#add centroid data to blockshp
# (i'm not sure if this is necessary to do anymore)
blocksShp.centroid= gCentroid(blocksShp,byid=T)
blocksShp@data$X = blocksShp.centroid@coords[,1]
blocksShp@data$Y = blocksShp.centroid@coords[,2]
...
#add centroid data to roads
# (i'm not sure if this is necessary to do anymore)
roadsShp = spTransform(roadsShp, proj4string(P17shp))
roadsShp.centroid = gCentroid(roadsShp,byid=T)
roadsShp@data$X = roadsShp.centroid@coords[,1]
roadsShp@data$Y = roadsShp.centroid@coords[,2]
...
#--------GETTING CENSUS DATA-------####
# we do a spatial overlay onto a census blocks shapefile, using the geographical information from the parcels shapefile 
# for the PA17 records
...
#only the non-duplicates are duplicated (since the geocoder needs a unique ID and the files that duplicate parcel_num have the same geography)
...
# getting all of the MAL records that can be connected using their parcel_num
...
# connecting old property assessment records geographically
...
#get the subset that linked to nothing
...
fullLink.f = merge(fullLink, walkover_ssa.d[,c("GIS_ID1","GIS_ID2")],by.x="GIS_ID",by.y="GIS_ID1",all=T)
fullLink.f = rename(fullLink.f, Land_Parcel_ID = GIS_ID2)



#writing the walkover
write.csv(fullLink.f,walkover_path,row.names=F)
...
# i'm still not positive why a very few parcel nums are repeated in the property assessment file, but it seems like it makes sense to drop them here, but keep them in the asssessment file
...
#merging the new ID onto the PA17 file 
# (before it was the PA17_b, because all we wanted to add was the ID, not we're adding it to the cleaned and modified one)
PA17 = merge(PA17,fullLink.f[!duplicated(fullLink.f$parcel_num),c("parcel_num","Land_Parcel_ID")],by="parcel_num",all.x=T)


#setting stuff up in PAC before creating parcels
...
### aggregating to create the properties file
...
numCondos = aggregate(parcel_num~Land_Parcel_ID, PA17[PA17$LU == "CD" | PA17$LU == "CC",], FUN = length)
...
for (num in c(1,2)) {
  ...
  #again, annoyingly not allowing multiple modes to be calculated at once, theres probably a way ive just been spending too long on this
  ...
  parcels.1 = merge(merge(parcels.1, PAC_sub_agg1, by="Land_Parcel_ID",all.x=T),PAC_sub_agg2, by="Land_Parcel_ID",all.x=T)
...
parcels.1$maxNum_1[ !is.na(parcels.1$maxNum_1) & !is.na(parcels.1$minNum_1) & parcels.1$maxNum_1 == parcels.1$minNum_1] = NA
parcels.1$maxNum_2[ !is.na(parcels.1$maxNum_2) & !is.na(parcels.1$minNum_2) & parcels.1$maxNum_2 == parcels.1$minNum_2] = NA
...
parcels.1 = merge(parcels.1, modal_geo, by="Land_Parcel_ID")
parcels.1 = parcels.1[,varnames]

#this should probably be incorporated back in earlier
parcels.1 = rename(parcels.1, zip = zip_c, street_1 = street_c_1, suffix_1 = suffix_c_1, street_2 = street_c_2, suffix_2 = suffix_c_2)

#writing the parcels file
write.csv(parcels.1,Land_Parcels_path,row.names=F)



# creating the merged shapefile and writing it
P17shp = merge(P17shp,fullLink.f[!duplicated(fullLink.f$GIS_ID),c("GIS_ID","Land_Parcel_ID")],by="GIS_ID",all.x=T)
